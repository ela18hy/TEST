...
class G_Node(QGraphicsEllipseItem):
    def __init__(self, x, y, r):
        ...
        # ============== Set Some Node Text ================
        self.textItem = QGraphicsSimpleTextItem("G", self)
    ...
    def mouseDoubleClickEvent(self, Event):
        if QMouseEvent.button() == Qt.LeftButton:
            seld.button_label.setText('Left')

    def contextMenuEvent(self, event):
        menu = QMenu()
        menu.addAction("Insert")
        menu.addAction("Colour")
...
class I_Node(QGraphicsEllipseItem):
    ...
    def mouseMoveEvent(self, event):
        ...
        # To understand the calculation, check out Coordinate.pdf
        # 需要了解计算方法的话请看 Coordinate.pdf
        updated_cursor_x = (
            updated_cursor_position.x() - orig_cursor_position.x() + orig_position.x()
        )
        updated_cursor_y = (
